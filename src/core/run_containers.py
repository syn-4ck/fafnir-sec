...
def _run_containers(client, tool, image, command, scan_fullpath, verbose, async_option, output_path, configuration):
    ...
    logging.info(f"Running {tool} scanning...")

    environment_vars = _setup_environment_vars(configuration, tool)

    if environment_vars:
        container = client.containers.run(image, command, volumes={scan_fullpath: {"bind": "/src", "mode": "rw"}, output_path: {
                                          "bind": "/report", "mode": "rw"}}, environment=environment_vars, detach=True, stdout=True)
    else:
        container = client.containers.run(image, command, volumes={scan_fullpath: {
                                          "bind": "/src", "mode": "rw"}, output_path: {"bind": "/report", "mode": "rw"}}, detach=True, stdout=True)
    ...
    if verbose and not async_option:
        try:
            ...
            logging.debug(f'{tool} execution finished')
    else:
        if not async_option:
...
def run_tools(client, config, scan_fullpath, verbose, configuration, async_option, output_path, tools):
    ...
    # Run Security tools
    logging.info("Running the Application security analysis")
            ...
            if tool in tools:
                if not os.path.exists(os.path.normpath(output_path + "/security-tools/" + tool)):
                    os.makedirs(os.path.normpath(
                        output_path + "/security-tools/" + tool))
                tool_config = config.get("containers").get(
                    "security-tools").get(category).get(tool)
                tool_image = "{img}:{version}".format(img=tool_config.get(
                    "image"), version=tool_config.get("version"))
                tool_command = tool_config.get("command")

                container_ids.append(_run_containers(client, tool, tool_image, tool_command,
                                     scan_fullpath, verbose, async_option, output_path, configuration))
    ...
    if configuration is None or configuration.get("containers") is None or configuration.get("containers").get("image") is None or configuration.get("containers").get("image") == "":
        ...
        for tool in config.get("containers").get("container-security"):
            if not os.path.exists(os.path.normpath(output_path + "/security-tools/" + tool)):
                os.makedir(os.path.normpath(
                    output_path + "/security-tools/" + tool))
            tool_config = config.get("containers").get(
                "container-security").get(tool)
            tool_image = "{img}:{version}".format(img=tool_config.get(
                "image"), version=tool_config.get("version"))
            tool_command = tool_config.get("command").format(
                configuration.get("containers").get("image"))
            container_ids.append(_run_containers(client, tool, tool_image, tool_command,
                                 scan_fullpath, verbose, async_option, output_path, configuration))
