...
def parse_semgrep_vulns(report: dict):
    ...
    for result in report.get('results', []):
        try:
            ...
            vulnerability.set_name(result.get('extra').get('metadata').get(
                'vulnerability_class')[0] + ' detected in your code')
            ...
            vulnerability.set_identifier(result.get(
                'extra').get('metadata').get('cwe')[0])
            vulnerability.set_severity(result.get(
                'extra').get('metadata').get('impact'))
