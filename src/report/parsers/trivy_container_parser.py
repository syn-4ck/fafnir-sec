...
def parse_trivy_container_vulns(report: dict):
    ...
    for source in report.get('Results'):
        for vuln in source.get('Vulnerabilities'):
            ...
            vulnerabilities.append(Vulnerability(name, vuln.get('Description'),vuln.get('VulnerabilityID'),vuln.get('Severity'),vuln.get('CVSS').get(list(vuln.get('CVSS').keys())[0]).get('V3Score'), "N/A",
